...
export class DataProvider {
 ...
 {
    let fetchPath = bibleBooks.filter(name => name.includes(bookName))[0];
    let book: BibleBook = null;
    this.http.get<BibleBook>(fetchPath).subscribe(response => 
    {
     book = response;
     this.bibleBookData = response;
      console.log("Book: "+book["verses"][0]);
